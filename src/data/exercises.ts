...
const screenshotMenuExercises: Exercise[] = [
  ...
  createMenuExercise({ id: "ankle-cars", nameZh: "踝关节 CARs", nameEn: "Ankle CARs", category: "foot-ankle", purpose: "用低负荷主动活动检查踝关节控制，不追求极限活动度。", youtubeSearchQuery: "ankle CARs controlled articular rotations", riskTier: "low", impactLevel: "none", safeAlternativeExerciseIds: ["ankle-knee-wall"] }),
  createMenuExercise({ id: "single-leg-balance", nameZh: "单脚平衡", nameEn: "Single-Leg Balance", category: "foot-ankle", purpose: "训练足三点、踝稳定和骨盆水平控制。", youtubeSearchQuery: "single leg balance exercise foot tripod", riskTier: "low", impactLevel: "none", safeAlternativeExerciseIds: ["short-foot"] }),
  createMenuExercise({ id: "slow-calf-raise", nameZh: "慢速提踵", nameEn: "Slow Calf Raise", category: "strength", purpose: "用慢速离心建立小腿和跟腱容量，避免弹性反复刺激。", youtubeSearchQuery: "slow calf raise eccentric technique", riskTier: "low", impactLevel: "low", safeAlternativeExerciseIds: ["calf-isometric-hold"] }),
  createMenuExercise({ id: "bent-knee-calf-isometric", nameZh: "屈膝提踵等长保持", nameEn: "Bent-Knee Calf Isometric", category: "isometric", purpose: "训练比目鱼肌和屈膝位跟腱可控负荷，作为左内侧踝敏感时的替代。", youtubeSearchQuery: "bent knee calf isometric hold soleus", riskTier: "low", impactLevel: "none", safeAlternativeExerciseIds: ["calf-isometric-hold"] }),
  createMenuExercise({ id: "trap-bar-speed-pull", nameZh: "Trap Bar 速度拉", nameEn: "Trap Bar Speed Pull", category: "power", purpose: "在不跳跃的情况下训练快速发力，用于替代踝痛时的负重跳。", youtubeSearchQuery: "trap bar speed pull technique", riskTier: "moderate", impactLevel: "low", safeAlternativeExerciseIds: ["trap-bar-deadlift", "kettlebell-swing"] }),
  createMenuExercise({ id: "banded-talus-mobilization", nameZh: "弹力带距骨松动", nameEn: "Banded Talus Mobilization", category: "mobility", purpose: "轻量检查踝背屈感觉，只在无痛时使用，不能强推活动度。", youtubeSearchQuery: "banded talus mobilization ankle dorsiflexion", riskTier: "low", impactLevel: "none", safeAlternativeExerciseIds: ["ankle-knee-wall"] }),
  createMenuExercise({ id: "low-landing-stick", nameZh: "低幅落地定住", nameEn: "Low Landing Stick", category: "plyometric", purpose: "用极低高度检查落地声音、膝脚对线和左内侧踝反应。", youtubeSearchQuery: "low landing stick drill quiet landing", riskTier: "low", impactLevel: "low", jumpContactContribution: { min: 3, max: 6, landingOnly: true }, safeAlternativeExerciseIds: ["step-down", "single-leg-balance"] }),
  createMenuExercise({ id: "landing-pain-check", nameZh: "落地疼痛检查", nameEn: "Landing Pain Check", category: "plyometric", purpose: "训练前用少量低幅落地判断是否可以进入弹性训练。", youtubeSearchQuery: "landing pain check plyometric warm up", riskTier: "low", impactLevel: "low", jumpContactContribution: { min: 3, max: 6, landingOnly: true }, safeAlternativeExerciseIds: ["step-down", "spanish-squat-isometric"] }),
  createMenuExercise({ id: "b-stance-rdl", nameZh: "B 站姿 RDL", nameEn: "B-Stance RDL", category: "strength", purpose: "用偏单侧髋铰链训练腘绳肌和髋稳定，比完全单腿更容易控制。", youtubeSearchQuery: "B stance Romanian deadlift technique", riskTier: "low", impactLevel: "low", safeAlternativeExerciseIds: ["rdl", "single-leg-rdl-top-lock"] }),
  createMenuExercise({ id: "cycle-review-log", nameZh: "周期复盘记录", nameEn: "Cycle Review Log", category: "recovery", purpose: "记录疼痛趋势、弹跳感觉、篮球负荷和下一阶段调整方向。", youtubeSearchQuery: "training review log athletic performance", riskTier: "low", impactLevel: "none", safeAlternativeExerciseIds: ["legs-up-breathing"] }),
  createMenuExercise({ id: "low-pogo-test", nameZh: "低幅 Pogo 疼痛阈值测试", nameEn: "Low Pogo Pain Threshold Test", category: "plyometric", purpose: "用最多 12 次低幅 Pogo 检查左内侧踝疼痛出现位置，不作为训练量。", youtubeSearchQuery: "low pogo jump ankle stiffness test", riskTier: "moderate", impactLevel: "low", jumpContactContribution: { min: 0, max: 12 }, safeAlternativeExerciseIds: ["bent-knee-calf-isometric", "single-leg-balance"] }),
  createMenuExercise({ id: "hanging-knee-raise", nameZh: "悬吊提膝", nameEn: "Hanging Knee Raise", category: "core", purpose: "训练骨盆后倾、核心抗伸展和髋屈控制，作为悬吊抬腿的温和版本。", youtubeSearchQuery: "hanging knee raise technique", riskTier: "low", impactLevel: "none", safeAlternativeExerciseIds: ["dead-bug", "hanging-abs-curl"] }),
