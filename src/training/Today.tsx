...
function TrainingMode({ session }: { session: Session }) {
  ...
  return <><Panel><Heading>{session.snapshot.title}</Heading><Copy>计划第{session.day}天 · 实际开始于 {session.date} · {sessionStatus(session)}</Copy><Copy>已记录跳跃 {actual.jumps} 次 · 左 {actual.left} / 右 {actual.right}</Copy>{!session.endedAt ? <Copy>组间休息：{remaining > 0 ? `剩余 ${remaining} 秒` : '计时结束 / 尚未开始'}</Copy> : null}</Panel>
    ...
    </Disclosure><Panel><Heading>训练后简短反馈</Heading><FeedbackForm session={session} /></Panel></>;
}
export default function Today() {
  ...
  return <Shell title={active ? '训练中' : '今天练什么'}>
    {session ? <TrainingMode session={session} /> : p ? <>
      ...
      {state.skippedDates.includes(date) ? <Panel><Copy>今天已跳过，不补课、不累计训练刺激。</Copy><Button title="撤销今日跳过" onPress={() => update(s => ({ ...s, skippedDates: s.skippedDates.filter(d => d !== date) }))} /></Panel> : <Button title={p.level === 'unassessed' ? '开始保守训练' : '开始今日训练'} primary onPress={() => update(s => startSession(s, date, new Date().toISOString()))} />}
